SimulationPlot <- function(result){
  ...
  fig <- ggplot(NULL, aes(value)) + 
    geom_density(aes(value, fill = "ergm"), data = gather(as.data.frame(coef_ergm)), alpha = 0.4) +
    geom_density(aes(value, fill = "nlls"), data = gather(as.data.frame(coef_nlls)), alpha = 0.2) +
