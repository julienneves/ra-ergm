...
net_mc <- function(params_true, psi, sigma_e, n, terms, repl = 100) {
  ...
  G_0 <- matrix(rbinom(n * n, 1, 0.15), n)
  diag(G_0) <- 0
  G_0 <- forceSymmetric(G_0)
  G_0 <-  matrix(G_0, n)
  G <- network(G_0)
  
  # Generate ERGM distribution using seed
  net_formation_true <- ergm(as.formula(paste("G ~ sender + receiver +", terms)))
  
  # Simulate a network using the true model distribution
  G <- simulate(net_formation_true)
  plot(G)
  
  # Compute xi from ERGM distribution
  xi <- true_model$coef[1:(2 * (n - 1))]
  ...
  y <- solve((diag(n) - phi * as.matrix(G)),  alpha + X %*% beta + psi * xi + epsilon)
  
  # Gather y, X in a data frame
  df <-  data.frame(y, X)
  
  # Estimate ERGM on G
  net_formation_est <- ergm(as.formula(paste("G ~ sender + receiver +", terms)))
  ...
  fit_nlls <- net_dep(formula = formula, df, G = as.matrix(G), model = "model_B", estimation = "NLLS",  hypothesis = "lim",start.val = params_true, endogeneity = FALSE)
  ...
  est_value <- data.frame(t(params_true))
  return(list(params_ergm, params_nlls, G = G))
}

## Example 1
# Monte Carlo simulation
params_true <- c(alpha = -1, beta_X = 1, phi = .5)
result <- net_mc(params_true, psi = 1, sigma_e = 1, n = 100, terms = c("triangle"), repl = 250)

# Plot coefficients
p1 <- ggplot(gather(result[[1]]), aes(value, fill=key)) + 
